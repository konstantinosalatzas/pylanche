...
class Client:
    def __init__(self):
        ...
        with open('./pylanche/event_hub.json', 'r') as file:
            ...
            self.EVENT_HUB_CONN_STR = event_hub_config['EVENT_HUB_CONN_STR']
            self.EVENT_HUB_NAME = event_hub_config['EVENT_HUB_NAME']
            self.RECEIVE_DURATION = event_hub_config['RECEIVE_DURATION']
            self.BLOB_STORAGE_CONN_STR = event_hub_config['BLOB_STORAGE_CONN_STR']
            self.BLOB_CONTAINER_NAME = event_hub_config['BLOB_CONTAINER_NAME']
            self.SEND_COUNT = event_hub_config['SEND_COUNT']

    def receive(self):
        receive(self.EVENT_HUB_CONN_STR, self.EVENT_HUB_NAME, self.RECEIVE_DURATION,
                self.BLOB_STORAGE_CONN_STR, self.BLOB_CONTAINER_NAME)

    def send(self):
        send(self.EVENT_HUB_CONN_STR, self.EVENT_HUB_NAME, self.SEND_COUNT)